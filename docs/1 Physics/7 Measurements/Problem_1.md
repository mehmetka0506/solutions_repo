...
length = 1.4  # metre
g = 9.81  # m/s^2
T = 2 * np.pi * np.sqrt(length / g)  # Periyot
frames_per_cycle = 20
num_cycles = 3
total_frames = frames_per_cycle * num_cycles
theta_max = np.radians(20)  # maksimum salınım açısı (derece → radyan)

# GIF dosyası için klasör
os.makedirs("frames", exist_ok=True)

# Her kare için görüntü oluştur
images = []
for i in range(total_frames):
    t = i / frames_per_cycle * T
    ...
    ax.plot([0, x], [0, y], 'k-', lw=2)  # ip
    bob = Circle((x, y), 0.05, fc='orange', ec='black')
    ax.add_patch(bob)
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    plt.text(-1.5, 0.1, 'Length = 1.40 m\n10 swings = 18 s', fontsize=12)
...
# Google Colab'da görüntüle
from IPython.display import Image as IPImage
IPImage(filename=gif_path)
```
![alt text](image-1.png)
[Colab Animation](https://colab.research.google.com/drive/1wkFwc-eaMsBnAOwcHkvCcEjHQi_VOJaX?usp=sharing)
